<script>
	...
	// Sende eine POST-Anfrage an den Server, um ein neues Konto zu erstellen
	fetch('/api/postAccount', {
		...
		.then((response) => {
			...
			if (response.ok) {
				...
				// Aktualisiere die accountList mit dem neuen Konto
				accountList = [
					...accountList,
					{
  ...
  try {
    // Überprüfen, ob ein Konto mit der angegebenen Bezeichnung existiert
    const accountToUpdate = accountList.find((account) => account.designation === designation);

    if (!accountToUpdate) {
      // Warnung in der Konsole ausgeben, wenn das Konto nicht gefunden wird
      console.warn(`Account with designation "${designation}" not found.`);
      return;
    }

    // Versuch, den Kontostand in der Datenbank auf null zu setzen
    ...
    // Lokale Konto-Liste aktualisieren
